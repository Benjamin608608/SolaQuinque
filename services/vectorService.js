                ...
                messages: [
                    {
                        role: "system",
                        content: useFastMode ? 
                            `您是神學知識庫助手。請嚴格基於提供的資料庫內容回答問題，不要添加任何資料庫中沒有的資訊。

嚴格要求：
1. 只能使用提供的資料庫內容回答
2. 如果資料庫中沒有相關資訊，請明確說明"資料庫中沒有找到相關資訊"
3. 使用 [1], [2], [3] 格式引用資料庫內容
4. 不要推測、不要添加外部知識
5. 保持學術性但簡潔` :
                            `您是一位專業的神學知識庫助手。請嚴格基於提供的資料庫內容回答問題，不要添加任何資料庫中沒有的資訊。

嚴格要求：
1. 只能使用提供的資料庫內容回答問題
2. 如果資料庫中沒有相關資訊，請明確說明"資料庫中沒有找到相關資訊"
3. 在回答中引用相關的來源，使用 [1], [2], [3] 等格式標註引用
4. 使用中文回答，保持傳統中文的表達方式
5. 不要推測、不要添加外部知識、不要自由發揮
6. 確保每個引用都有對應的資料庫來源

重要：請嚴格基於提供的資料庫內容回答，如果資料庫中沒有相關資訊，請明確說明。`
                    },
                    {
                        role: "user",
                        content: `問題：${query}
...
內容：${result.text}`).join('\n\n')}`
            ...
            } else if (vectorResults.length > 0 && answer.includes("資料庫中沒有找到相關資訊")) {
                // 如果 AI 說沒有找到，但實際上我們有資料，強制使用資料庫內容
                answer = `根據資料庫中的相關資料：
